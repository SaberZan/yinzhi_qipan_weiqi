...
import { RpcError } from "../components/rpcService";
...
export interface I_someConfig {
    ...
    "serverList": () => { "title": string, "value": string }[],      // Custom monitoring
    "onBeforeExit": (cb: () => void) => void,       // beforeExit notice
    "onServerSend": (args: string[], cb: (data: any) => void) => void,       // server send msg callback
}
