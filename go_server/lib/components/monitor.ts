...
export class monitor_client_proxy {
    ...
    private doConnect(delay: number) {
        ...
        setTimeout(function () {
            let connectCb = function () {
                self.app.logger("server", loggerLevel.debug, `${meFilename} connected to master success`);
            ...
            self.app.logger("server", loggerLevel.debug, `${meFilename} try to connect to master now`);
     ...
     */
    private register() {
        let tokenConfig = this.app.someconfig.recognizeToken || { "serverToken": undefined, "cliToken": undefined };
     ...
     */
    private onClose() {
        this.app.logger("server", loggerLevel.error, `${meFilename} socket closed, try to reconnect master later`);
        this.needDiff = true;
    ...
    private heartbeatTimeout() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
            self.app.logger("server", loggerLevel.error, `${meFilename} heartbeat timeout, close the socket`);
            self.socket.close();
