...
export function decode(socket: SocketProxy, msg: Buffer) {
    ...
    while (readLen < msg.length) {
        if (socket.len === 0) // data length is unknown
        {
            ...
            if (socket.headLen === 4) {
                ...
                if (socket.len > socket.maxLen || socket.len === 0) {
                    app.logger("server", loggerLevel.error, `${meFilename} socket data length is wrong, close it, ${socket.remoteAddress}`);
                    socket.close();
                    return;
                }
        ...
        {
            socket.buffer.set(msg.slice(readLen), socket.buffer.length - socket.len);
            socket.len -= (msg.length - readLen);
            readLen = msg.length;
        }
        else { // data coming all
            socket.buffer.set(msg.slice(readLen, readLen + socket.len), socket.buffer.length - socket.len);
