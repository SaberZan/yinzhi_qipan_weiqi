...
class RpcServerSocket {
    ...
    constructor(app: Application, socket: SocketProxy) {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("server", loggerLevel.error, `${meFilename} register timeout, close the rpc socket: ${socket.remoteAddress}`);
            socket.close();
        }, 5000);
    }
    ...
    private onRegisterData(data: Buffer) {
        try {
            ...
            if (type === define.Rpc_Msg.register) {
                ...
                this.app.logger("server", loggerLevel.error, `${meFilename} illegal rpc register, close the rpc socket: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        } catch (e: any) {
            this.socket.close();
            this.app.logger("server", loggerLevel.error, e);
        }
    }
        ...
        try {
            ...
            switch (type) {
                ...
                case define.Rpc_Msg.heartbeat:
                    ...
                    break;
                default:
                    this.app.logger("server", loggerLevel.error, `${meFilename} illegal data type, close rpc client named: ${this.id}`);
                    this.socket.close();
                    break;
            }
        } catch (e: any) {
            this.app.logger("server", loggerLevel.error, e.stack);
        }
    }
    ...
    private onClose() {
        ...
        this.app.logger("server", loggerLevel.error, `${meFilename} a rpc client disconnected: ${this.id}, ${this.socket.remoteAddress}`);
    }
    ...
    private registerHandle(msg: Buffer) {
        ...
        try {
            ...
            this.app.logger("server", loggerLevel.error, `${meFilename} JSON parse error，close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }

        if (data.serverToken !== serverToken) {
            this.app.logger("server", loggerLevel.error, `${meFilename} illegal serverToken, close the rpc socket: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        if (this.app.rpcPool.getSocket(data.id)) {
            this.app.logger("server", loggerLevel.error, `${meFilename} already has a rpc client named: ${data.id}, close it, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        ...
        this.app.logger("server", loggerLevel.debug, `${meFilename} get new rpc client named: ${this.id}`);
    ...
    private heartbeatHandle() {
        ...
        this.heartbeatTimer = setTimeout(() => {
            this.app.logger("server", loggerLevel.error, `${meFilename} heartBeat time out, close it: ${this.id}`);
            this.socket.close();
        }, heartbeat * 1000 * 2);
    }
    ...
    private sendInterval() {
        if (this.sendArr.length > 0) {
            this.socket.send(Buffer.concat(this.sendArr.map((buf) => new Uint8Array(buf))));
