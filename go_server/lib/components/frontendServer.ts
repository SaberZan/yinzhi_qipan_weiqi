...
export class FrontendServer {
    ...
    start(cb: Function) {
        ...
        let myServer = require("../myServer");
        let connectorConfig = this.app.someconfig.connector || {};
        let connectorConstructor: I_connectorConstructor = connectorConfig.connector || myServer.connector.Tcp;
...
class ClientManager implements I_clientManager {
    ...
    addClient(client: I_clientSocket) {
        if (client.session) {
            this.app.logger("server", loggerLevel.error, `${meFilename} the I_client has already been added, close it`);
            client.close();
            return;
        }
        this.app.clientNum++;
    ...
    handleMsg(client: I_clientSocket, msgBuf: Buffer) {
        try {
            if (!client.session) {
                this.app.logger("server", loggerLevel.error, `${meFilename} cannot handle msg before added, close it`);
                client.close();
                return;
            }
    ...
    private doRemote(msg: { "cmd": number, "msg": Buffer }, session: Session, cmdArr: string[]) {
        ...
        if (svr.serverType !== cmdArr[0] || svr.frontend) {
            this.app.logger("server", loggerLevel.error, `${meFilename} illegal doRemote`);
            return;
        }
        ...
        buf.set(sessionBuf, 7);
        buf.writeUInt16BE(msg.cmd, 7 + sessionBuf.length);
        buf.set(msg.msg, 9 + sessionBuf.length);
        socket.send(buf);
    }
}