...
export class RpcClientSocket {
    ...
    private doConnect(delay: number) {
        ...
        this.connectTimer = setTimeout(() => {
            let connectCb = function () {
                self.app.logger("server", loggerLevel.debug, `${meFilename} connect to rpc server success: ${self.id}`);
            ...
            self.app.logger("server", loggerLevel.debug, `${meFilename} try to connect to rpc server: ${self.id}`);
        }, delay);
    }
    ...
    private onClose() {
        ...
        this.app.logger("server", loggerLevel.error, `${meFilename} socket closed, reconnect the rpc server later: ${this.id}`);
    ...
    private heartbeatTimeoutStart() {
        ...
        this.heartbeatTimeoutTimer = setTimeout(function () {
            self.app.logger("server", loggerLevel.error, `${meFilename} heartbeat timeout, close the rpc socket: ${self.id}`);
            self.socket.close();
