...
function startServer(cb?: Function) {
    let tokenConfig = app.someconfig.recognizeToken || { "serverToken": undefined, "cliToken": undefined };
...
class UnregSocket_proxy {
    ...
    private registerTimeout() {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, register timeout, close it, ${self.socket.remoteAddress}`);
            self.socket.close();
        }, 5000);
    ...
    private onData(_data: Buffer) {
        ...
        try {
            ...
            app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, JSON parse error, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        ...
        if (!data || data.T !== define.Monitor_To_Master.register) {
            app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, illegal data, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        ...
        if (data.serverToken) {
            if (data.serverToken !== serverToken) {
                app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, illegal serverToken, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            if (!data.serverInfo || !data.serverInfo.id || !data.serverInfo.host || !data.serverInfo.port || !data.serverInfo.serverType) {
                app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, illegal serverInfo, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        if (data.cliToken) {
            if (data.cliToken !== cliToken) {
                app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, illegal cliToken, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        app.logger("server", loggerLevel.error, `${meFilename} unregistered socket, illegal socket, close it, ${socket.remoteAddress}`);
        socket.close();
    }

    private onClose() {
        clearTimeout(this.registerTimer);
        app.logger("server", loggerLevel.error, `${meFilename} unregistered socket closed, ${this.socket.remoteAddress}`);
    }
...
export class Master_ServerProxy {
    ...
    private init(data: monitor_reg_master) {
        ...
        if (!!servers[data.serverInfo.id]) {
            app.logger("server", loggerLevel.error, `${meFilename} already has a monitor named: ${data.serverInfo.id}, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        ...
        app.logger("server", loggerLevel.debug, `${meFilename} get a new monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
    }

    private heartbeatTimeout() {
        this.heartbeatTimeoutTimer = setTimeout(() => {
            app.logger("server", loggerLevel.error, `${meFilename} heartbeat timeout, close the monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
            this.socket.close();
    ...
    private onData(_data: Buffer) {
        ...
        try {
            ...
            app.logger("server", loggerLevel.error, `${meFilename} JSON parse error，close the monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        ...
        try {
            if (data.T === define.Monitor_To_Master.heartbeat) {
                ...
                app.logger("server", loggerLevel.error, `${meFilename} the monitor illegal data type close it: ${this.sid} ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        } catch (e: any) {
            app.logger("server", loggerLevel.error, e);
            this.socket.close();
        }
    }
    ...
    private onClose() {
        ...
        app.logger("server", loggerLevel.error, `${meFilename} a monitor disconnected: ${this.sid}, ${this.socket.remoteAddress}`);
    }
}
...
export class Master_ClientProxy {
    ...
    private init() {
        ...
        app.logger("server", loggerLevel.info, `${meFilename}  get a new cli: ${socket.remoteAddress}`);
    }

    private heartbeatTimeOut() {
        this.heartbeatTimeoutTimer = setTimeout(() => {
            app.logger("server", loggerLevel.error, `${meFilename} heartbeat timeout, close the cli: ${this.socket.remoteAddress}`);
            this.socket.close();
        }, define.some_config.Time.Monitor_Heart_Beat_Time * 1000 * 2);
    }
    ...
    private onData(_data: Buffer) {
        ...
        try {
            ...
            app.logger("server", loggerLevel.error, `${meFilename} JSON parse error，close the cli: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        ...
        try {
            if (data.T === define.Cli_To_Master.heartbeat) {
                ...
                app.logger("server", loggerLevel.info, `${meFilename} get command from the cli: ${this.socket.remoteAddress} ==> ${_data.toString()}`);
                masterCli.deal_cli_msg(this, data);
            } else {
                app.logger("server", loggerLevel.error, `${meFilename} the cli illegal data type close it: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        } catch (e: any) {
            app.logger("server", loggerLevel.error, `${meFilename} cli handle msg err, close it: ${this.socket.remoteAddress}\n ${e.stack}`);
            this.socket.close();
        }
    }
    ...
    private onClose() {
        clearTimeout(this.heartbeatTimeoutTimer);
        app.logger("server", loggerLevel.info, `${meFilename}  a cli disconnected: ${this.socket.remoteAddress}`);
    }
}