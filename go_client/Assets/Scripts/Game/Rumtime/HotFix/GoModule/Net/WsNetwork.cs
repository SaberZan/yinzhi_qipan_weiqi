...
public static class WsNetwork
{
    ...
    /// 断开socket连接
    /// </summary>
    public static async EasyEmptyTask DisConnect()
    {
        if (nowSocket != null)
        {
            await nowSocket.DisConnect();
        }
        lock (lockObj)
        {
            msgCache.Clear();
    ...
    public static async EasyEmptyTask Connect(string host, int port)
    {
        await DisConnect();
        nowSocket = new NetworkChild();
        await nowSocket.Connect(host, port);
    }
    ...
    {
        private ClientWebSocket mySocket = null;         //原生socket
        private bool isDead = false;            //是否已被弃用
        private System.Timers.Timer heartbeatTimer = null;    // 心跳
        private System.Timers.Timer heartbeatTimeoutTimer = null;    // 心跳回应超时
        ...
        public async EasyEmptyTask DisConnect()
        {
            if (!isDead)
            {
                ...
                try
                {
                    ...
                    {
                        await mySocket.CloseAsync(WebSocketCloseStatus.Empty, null, cancellationTokenSource.Token);
                        await EasyTaskRunner.WaitUntil(()=>mySocket.State == WebSocketState.Closed);
                    }
                }
                catch (Exception e)
                {
        ...
        public void Send(int cmdIndex, string data)
        {
            ...
            try
            {
                mySocket.SendAsync(new ArraySegment<byte>(bytes), WebSocketMessageType.Binary, true, cancellationTokenSource.Token);
            }
            catch (Exception e)
            {
                Debug.Log(e);
        ...
        public async EasyEmptyTask Connect(string host, int port)
        {
            try
            {
                mySocket = new ClientWebSocket();
                await mySocket.ConnectAsync(new Uri($"ws://{host}:{port}"), cancellationTokenSource.Token);
                await AsyncConnectCallback();
            }
            catch (Exception e)
            {
                Debug.Log(e);
        ...
        private async EasyEmptyTask AsyncConnectCallback()
        {
            ...
            {   // 异步写入结束 
                ...
                await mySocket.SendAsync(new ArraySegment<byte>(byteEnd), WebSocketMessageType.Binary, true, cancellationTokenSource.Token);
            }
            catch (Exception e)
            {
                Debug.Log(e);
        ...
        private async void Recive()
        {
            try
            {
                ...
                if(mySocket.State == WebSocketState.Open)
                {
                    var result = await mySocket.ReceiveAsync(new ArraySegment<byte>(data), cancellationTokenSource.Token);
                ...
                else if(mySocket.State == WebSocketState.Closed)
                {
                    return;
                }
                Recive();
        ...
        private async EasyVoidTask SocketClose()
        {
            if (!isDead)
            {
                SocketMsg msg = new SocketMsg();
